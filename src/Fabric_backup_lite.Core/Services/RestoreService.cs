        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var backupItem in metadata.Items)
        {
            ...
            items.Add(new RestoreItemViewModel
            {
                ...
                IsChecked   = isRestorable,
                TypeIcon    = GetItemIcon(type),
                TooltipText = isRestorable
                    ? string.Empty
                    : "Warehouse restore is not supported via API."
            });
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var vm in selectedItems)
        {
            ...
            progress?.Report(new RestoreProgress
            {
                ...
                Message        = string.Format("Restoring {0}: {1}...", itemType, itemName)
            });
            ...
            {
                var errorMsg = string.Format("Error restoring '{0}': {1}", itemName, ex.Message);
