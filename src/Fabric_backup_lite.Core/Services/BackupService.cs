...
public class BackupService : IBackupService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var finalMessage = result.Success
                ? string.Format("Backup completed: {0} items backed up.", result.ItemsBackedUp)
                : string.Format("Backup completed with errors: {0} items backed up, {1} errors.", result.ItemsBackedUp, result.Errors.Count);
        ...
        CancellationToken cancellationToken = default)
    {
        if (selectedItems.Count == 0)
        {
            ...
            {
                Success = false,
                Errors  = { "No items selected for backup." }
            };
        }
        ...
        ReportProgress(progress, total, 0, "", string.Format("Starting backup of {0} items...", total));
            ...
            foreach (var (_, _, item) in wsGroup)
            {
                ...
                try
                {
                    ReportProgress(progress, total, completed, item.DisplayName,
                        string.Format("Saving {0}: {1}...", item.Type, item.DisplayName));
                ...
                {
                    var err = string.Format("Error backing up '{0}': {1}", item.DisplayName, ex.Message);
